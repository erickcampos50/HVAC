...
t_const = 23  //Temperatura de bulbo seco
um_rel = 0.4 //Umidade relativa
...
q_ilum_pot = 1200 //Potencia das lampadas e equipamentos eletronicos equivalentes ligados no mesmo horario. Para A201 a principal carga são 30 lampadas (40W)

rts_ilum_sel = 1 //indice pra selecao do rts mais apropriado (a principio apenas valores da tabela medium)
...
alpha(1)=strtod(alpha_tab(12,2))

w = [
-25,0000	90,0000	1,0000	0.45	20,0400	12,9600
-75,0000	90,0000	1,0000	0.45	18,5040	12,0960
0,0000	0,0000	1,0000	0.45	112,2000	0,0000]


for i = 1:size(w,1)
q_cond_pico_total = 0 //Deve permanecer dentro do loop
...
p = 40 //Ocupação em horario de pico ou ocupacao media
//Horario de ocupacao do amiente
hor_ocup_i = 7   //Horario de funcionamento inicio
hor_ocup_f = 18  //Horario de funcionamento fim
...
pma = 13 //Porta mal ajustada m³/h por metro linear de fresta NBR 6401
pba = 6.5 //Porta bem ajustada m³/h
fresta = 10 //Estimativa de portas e principalmente janelas, que estão mal ajustadas

porta = 14//Parcela correspondente a abertura e fechamento das portas m3/h/pessoa
...
q_wind_pico_total = 0 //Deve permanecer dentro do loop
...
plot(q_total,'r');plot(q_wind_total,'g');plot(q_infilt_total,'b');plot(q_ocup_total,'c');plot(q_cond_pico_total,'y');plot(q_ilum_total,'k');

//Substituir valores de area e U para cada material
//Criar versão de calculo considerando sombreamento nas paredes e janelas, quando houver sombra é preciso considerar somente as parcelas condutivas e de ensolacao difusa nas paredes e janelas
